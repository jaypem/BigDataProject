...
def load_api():
    ...
    consumer_key = 'ZglAuCnnlzcLehRfgYzn1UKKh'
    consumer_secret = 'RmnD8C4ABxfVvFlgvbDZhdixSJjpAtk5jKwev77IfwC0epYLGA'
    access_token = '4332790276-nJsa9XeZhGEjaMOLV0ZjuYP1rfQWhl9vuO61Jb3'
    access_secret = 'N5NZZcJRgx5bQbmHPxdazIznNJpurqfpCBRidpAFhvljN'
...
def get_tweet_id(api, date='', days_ago=9, query='a'):#days_ago=9, query='a'):
    ...
    if date: # return an ID from the start of the given day
        ...
        # print(str(td).split()[0])
        # tweet_date = '{0}-{1:0&amp;gt;2}-{2:0&amp;gt;2}'.format(td.year, td.month, td.day)
        tweet_date = str(td).split()[0]
...
def write_tweets(tweets, filename):
    ...
    with open(filename, 'a') as f:
        for tweet in tweets:
            ...
            # json.dump(tweet._json, f, ensure_ascii=False).encode('utf8')
            f.write('\n')
            # f.write(',\n')
...
#     api = load_api()
#
#     days_ago, days_to = 205, 22
#     date = 2018-1-21
#
#     td = dt.datetime.now() - dt.timedelta(days=days_to)
#     tweet_date = '{0}-{1:0>2}-{2:0>2}'.format(td.year, td.month, td.day)
#     # get list of up to 10 tweets
#     tweet = api.search(q='itcoin', until=date)
#     # print('search limit (start/stop):',tweet[0].created_at)
#     print(len(tweet))
#     # tweet = api.search(search_phrase, geocode=USA)
...
def main():
    ...
    ''' search variables: '''
    search_phrases = ['Bitcoin', 'bitcoin',
                      '#Bitcoin', '#bitcoin',
